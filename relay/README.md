...
2. Railway auto-detects Node (Nixpacks) and runs `npm start`. `PORT` is provided
   ...
   — it protects the official Kick alerts route (`kick-alerts` subscribe) and
   the `/kick/authorize` setup step. Kick **chat** (`platform:'kick'`) never
   requires a token, so old/other widgets keep working even if you rotate it.
...
- **Kick channel**: your channel **slug** (e.g. `xqc`) **or** the numeric
  **chatroom id**.
- **Relay access token**: not needed for chat — chat subscribes are never
  gated. This field only matters for the Kick alerts widget
  (`widget-kick-alerts`), which does require it when `RELAY_TOKEN` is set.
